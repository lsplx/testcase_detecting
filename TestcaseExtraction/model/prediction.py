...
def _convert_pred_entities(entity_types: torch.tensor, entity_spans: torch.tensor,
    ...
    # print(entity_types)
    # print(valid_entity_indices)
    # print(type(valid_entity_indices))
...
def store_predictions(documents,sentence, pred_entities, pred_relations, store_path):
    predictions = []
    ...
    for i, doc in enumerate(documents):
        ...
        # 新加softmax
        # sample_entity_vec = entity_veclist[i]
        # convert entities
        converted_entities = []
        num = 0
        for j,entity in enumerate(sample_pred_entities):
            ...
            for span_token in span_tokens:
                entity_value = entity_value + " " + span_token.phrase
            entity_type = entity[2].identifier
            entity_pred = entity[3]
            entity_pred = round(entity_pred, 2)
            tensorstr = ""
            # for tensor in entity_veclist[j]:
            #     tensorstr =  str(tensor) + ";" + tensorstr 
            # print(tensorstr)
            converted_entity = dict(id = num , type=entity_type, value=entity_value, pred=entity_pred, startIndex = startIndex - 1, endIndex = endIndex - 1)
        ...
        for relation in sample_pred_relations:
            ...
            head_entity_value = ""
            for span_token in head_span_tokens:
                head_entity_value = head_entity_value + " " + span_token.phrase
            tail_span_tokens = util.get_span_tokens(tokens, tail_span)
            tail_entity_value = ""
            for span_token in tail_span_tokens:
                tail_entity_value = tail_entity_value + " " + span_token.phrase
            relation_type = relation[2].identifier
            print("relationpred:")
            print(relation[3])
            relationpred = round(relation[3], 2)
            for entity in converted_entities:
                if entity["startIndex"] == head[0]-1:
                    headIndex = entity["id"]
                if entity["startIndex"] == tail[0]-1:
                    tailIndex = entity["id"]
            converted_relation = dict(type=relation_type, head=head_entity_value, tail=tail_entity_value, headIndex = headIndex, tailIndex = tailIndex, pred = relationpred)
            converted_relations.append(converted_relation)
        # converted_relations = sorted(converted_relations, key=lambda r: r['head'])

        doc_predictions = dict(tokens=[t.phrase for t in tokens],sentence=sentence, entities=converted_entities,
