...
from transformers import BertConfig,XLNetConfig,AlbertConfig
...
def get_token(h: torch.tensor, x: torch.tensor, token: int):
    ...
    # if token_h.shape[0] <= x.shape[-1]:
    #     indices = torch.LongTensor([0])
    # else:
    #     indices = torch.LongTensor([0, x.shape[-1]])
    
    # token_h = torch.index_select(token_h, 0, indices)
